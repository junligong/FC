...
- 每次执行先回读根 `../../AGENTS.md` 的“浏览器强制规则”；来源核验先运行 `node automation/browser-triage.mjs`，只走 `web-access` 的 CDP Proxy 通道（独立调试 profile `Chrome-FC-Debug` :9333），不用日常 Chrome、IAB、临时 profile 或 extension 模式。
