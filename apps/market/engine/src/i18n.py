#!/usr/bin/env python3
"""
作用：为Python市场报告提供与Node版本一致的中英文文案切换。

Mirrors src/i18n.mjs. Supports three modes:
  'zh'        — Chinese only
  'en'        — English only
...
class I18n:
    ...
    def labels(self, keys, lang: Optional[str] = None):
        """Get all labels for a list of keys."""
        return [self.t(k, lang) for k in keys]


def create_i18n(default_lang: str = "zh") -> I18n:
...
def read_language_from_config(config_path: str = "config.json") -> str:
