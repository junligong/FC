...
- 每次执行先回读根 `../../AGENTS.md` 的“浏览器强制规则”；FUTBIN 只能通过 `web-access` 的 CDP Proxy 连接独立调试 profile `Chrome-FC-Debug` 的 9333 端口，唯一前置判据是 `node automation/browser-triage.mjs` 退出码。禁止日常 Chrome、IAB、临时 profile 和 extension 模式。
