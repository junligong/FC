...
- 单项只生成自身报告，15 分钟内完成或提交部分结果。资讯、足球、市场、进化都通过 `Web Access（浏览器自动化）` 技能（CDP Proxy 直连独立调试 profile `Chrome-FC-Debug` :9333）采集；资讯采集阶段最多 10 分钟。足球是 AI 采集，15 分钟是执行指令预算，并非平台硬限制。
