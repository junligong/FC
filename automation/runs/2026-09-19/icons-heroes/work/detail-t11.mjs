// 一次性明细脚本（临时）：icons-pricerange-hourly 2026-09-19 T11 轮建议集变动与临界余量明细。
// 全部按 cardId 关联（FUTBIN slug 不唯一，禁用 slug/name 做键）。
// 双映射：复算前轮(T10)命中集用「生成 T10 产物时的同一版 FC26 映射」(research-prev-t10.json)，避免 slug 漂移 artifact。
...
const hNow = j(`apps/market/engine/icons/data/prices/fc27/pricerange/hourly/${D}T11.json`);
...
const pres = j('automation/runs/2026-09-19/icons-heroes/work/research-prev-t10.json');
...
P(`建议集 T10 ${hitPrev.length} → T11 ${hitNow.length}（进 ${inNow.length} 出 ${outNow.length}）`);
P('--- 进 T11（用本轮映射看当前值）---');
for (const id of inNow) P('  ' + line(id, nowById.get(id), mapNow));
P('--- 出 T11（前轮值 vs 本轮值，同一映射=本轮）---');
