// 一次性明细脚本（临时）：icons-pricerange-hourly 2026-09-19 T05 轮建议集变动与临界余量明细。
// 全部按 cardId 关联（FUTBIN slug 不唯一，禁用 slug/name 做键）。
// 双映射：复算前轮(T04)命中集用「生成 T04 产物时的同一版 FC26 映射」(research-prev-t04.json)，避免 slug 漂移 artifact。
...
const hPrev = j(`apps/market/engine/icons/data/prices/fc27/pricerange/hourly/${D}T04.json`);
const res = j(`apps/market/engine/icons/data/research/fc26-vs-fc27-${D}.json`);
const pres = j('automation/runs/2026-09-19/icons-heroes/work/research-prev-t04.json');
...
P(`建议集 T04 ${hitPrev.length} → T05 ${hitNow.length}（进 ${inNow.length} 出 ${outNow.length}）`);
P('--- 进 T05（用本轮映射看当前值）---');
