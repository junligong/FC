// 一次性校验脚本（临时）：icons-pricerange-hourly 2026-09-19 T00 轮产物一致性核对（v2，字段结构已核正）。
// 全部跨文件关联一律按 cardId（FUTBIN slug 不唯一，禁用 slug/name 关联）。
// 输入：current.json / hourly T00 / hourly T23(前轮) / daily 2026-09-19 / research JSON / base-icons.json / 两个 HTML
...
P(`[快照] T23 cards=${prevCards.length} 唯一id=${prevById.size} collectedAt=${hPrev.collectedAt}`);
...
let rangeChg = 0, repChg = 0, newValid = 0, lostValid = 0;
...
P(`[T23→T${HH}] 区间变动=${rangeChg} 张 ${rangeChgList.join(' | ')}`);
P(`[T23→T${HH}] 代表价变动=${repChg} 张 · 新转有效=${newValid} · 有效转无效=${lostValid} · 任一平台有效 ${prevValidAny} → ${anyValid}`);
P(`[T23→T${HH}] 代表价变动样例：${repChgList.join(' | ')}`);

// ---- 研究判定链独立重算（按 cardId） ----
const fc26ById = new Map(res.rows.map(r => [String(r.id), r.fc26Launch]));
const recompute = (byId) => {
  const hit = [];
  for (const [id, c] of byId) {
    const f = fc26ById.get(id);
...
const hitNow = recompute(nowById), hitPrev = recompute(prevById);
...
P(`[研究] 独立重算 T23 命中=${hitPrev.length} 张（用于证明判定链无回归）`);
...
P(`[研究] 建议集 T23 ${hitPrev.length} → T${HH} ${hitNow.length}（进 ${inNow.length} 出 ${outNow.length}）`);
