// 一次性明细脚本（临时）：icons-pricerange-hourly 2026-09-19 T22 轮建议集变动与临界余量明细。
// 全部按 cardId 关联（FUTBIN slug 不唯一，禁用 slug/name 做键）。
// T22 本轮为全量成功轮（131/131，无 ok=false）：本轮取值口径 = current.json（分析器实际读取的源），
// 前轮取值 = T21 逐小时快照（同为全量成功轮）。
// 双映射：复算前轮(T21)命中集用生成该轮产物时的同一版 FC26 映射（research-prev-t21.json）。
...
const hNow = j(`apps/market/engine/icons/data/prices/fc27/pricerange/hourly/${D}T22.json`);
...
P(`建议集 T21 ${hitPrev.length} → T22 ${hitNow.length}（进 ${inNow.length} 出 ${outNow.length}）`);
