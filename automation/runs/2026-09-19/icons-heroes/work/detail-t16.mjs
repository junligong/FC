// 一次性明细脚本（临时）：icons-pricerange-hourly 2026-09-19 T16 轮建议集变动与临界余量明细。
// 全部按 cardId 关联（FUTBIN slug 不唯一，禁用 slug/name 做键）。
// T16 为「部分失败」轮（首批 8 张 403）：本轮取值口径 = current.json（分析器实际读取的源，
// 未采到卡的保留上次观测），前轮取值 = T13 逐小时快照（上一次全量成功轮）。
// 双映射：复算前轮(T13)命中集用生成该轮产物时的同一版 FC26 映射（research-prev-t15.json）。
...
const hNow = j(`apps/market/engine/icons/data/prices/fc27/pricerange/hourly/${D}T16.json`);
const hPrev = j(`apps/market/engine/icons/data/prices/fc27/pricerange/hourly/${D}T13.json`);
const res = j(`apps/market/engine/icons/data/research/fc26-vs-fc27-${D}.json`);
const pres = j('automation/runs/2026-09-19/icons-heroes/work/research-prev-t15.json');
...
P(`建议集 T13 ${hitPrev.length} → T16 ${hitNow.length}（进 ${inNow.length} 出 ${outNow.length}）`);
