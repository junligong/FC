// 一次性脚本（临时）：T21 轮「前、本轮命中集」按分析器真实输入（research rows）自洽复算，
// 作为「判定链无回归」的独立证据（不依赖 hourly 快照口径，避免部分失败轮造成的假差异）。
// 口径：命中集 = 从 research rows 的 fc27Current.representative / fc27Max / fc26Launch 复算。
// 输入：research（T21）+ research-prev-t20.json（T20）+ hourly T20（仅用于核查该轮是否含未采集卡）。
...
const pres = j('automation/runs/2026-09-19/icons-heroes/work/research-prev-t20.json');
const hPrev = j(`apps/market/engine/icons/data/prices/fc27/pricerange/hourly/${D}T20.json`);
...
P(`[自洽复算] T20(前轮) 文件自判=${advPrevFile.length} 与 rows 复算=${hitPrevRows.length} 差异=${d(hitPrevRows, advPrevFile)}`);
P(`[自洽复算] T21(本轮) 文件自判=${advNowFile.length} 与 rows 复算=${hitNowRows.length} 差异=${d(hitNowRows, advNowFile)}`);
...
P(`[基线核查] T20 未采集卡=${unfetched.length} 张${unfetched.length ? '：' + unfetched.map((id) => pres.rows.find((r) => String(r.id) === id)?.nameZh || id).join(' / ') : '（T20 为全量成功轮 ⇒ hourly 基线本轮同样可信）'}`);
const affected = unfetched.filter((id) => advPrevFile.includes(id));
P(`[基线核查] 未采集但在 T20 命中集内=${affected.length} 张${affected.length ? '：' + affected.map((id) => `${pres.rows.find((r) => String(r.id) === id)?.nameZh}(${id}) T20保留观测 con${prevCards.get(id)?.current?.console} pc${prevCards.get(id)?.current?.pc}`).join(' | ') : ''}`);

// 真实进/出（以 rows 自洽复算的 T20 命中集为基线）
const inNow = hitNowRows.filter((x) => !hitPrevRows.includes(x));
const outNow = hitPrevRows.filter((x) => !hitNowRows.includes(x));
P(`[真实变动] 建议集 T20 ${hitPrevRows.length} → T21 ${hitNowRows.length}（进 ${inNow.length} 出 ${outNow.length}）`);
