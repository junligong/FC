// 一次性校验脚本（临时）：icons-pricerange-hourly 2026-09-19 T07 轮产物一致性核对（v2，字段结构已核正）。
// 全部跨文件关联一律按 cardId（FUTBIN slug 不唯一，禁用 slug/name 关联）。
// 输入：current.json / hourly T00 / hourly 前轮(前轮) / daily 2026-09-19 / research JSON / base-icons.json / 两个 HTML
...
P(`[前轮→T${HH}] 代表价变动=${repChg} 张 · 新转有效=${newValid} · 有效转无效=${lostValid} · 任一平台有效 ${prevValidAny} → ${anyValid}`);
...
const hitNow = recompute(nowById), hitPrev = recompute(prevById, fc26ByIdPrev);
