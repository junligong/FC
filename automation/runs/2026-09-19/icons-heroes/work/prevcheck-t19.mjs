...
// 口径：前轮命中集一律从 research-prev-t18.json 的 rows（= T16 分析器实际读取的 fc27Current/priceRange + fc26Launch）复算，
...
const pres = j('automation/runs/2026-09-19/icons-heroes/work/research-prev-t18.json');
const hPrev = j(`apps/market/engine/icons/data/prices/fc27/pricerange/hourly/${D}T18.json`);
...
P(`[自洽复算] T18(前轮) 文件自判=${advPrevFile.length} 与 rows 复算=${hitPrevRows.length} 差异=${d(hitPrevRows, advPrevFile)}`);
...
// 定位假差异：T18 快照未采集卡（current 两平台均为 0）在「快照口径复算」中会丢失
const prevCards = new Map((hPrev.cards || []).map((c) => [String(c.id), c]));
const unfetched = (hPrev.cards || []).filter((c) => c.ok === false).map((c) => String(c.id));
P(`[假差异根因] T18 未采集卡=${unfetched.length} 张：${unfetched.map((id) => pres.rows.find((r) => String(r.id) === id)?.nameZh || id).join(' / ')}`);
const affected = unfetched.filter((id) => advPrevFile.includes(id));
P(`[假差异清单] 未采集但在 T18 命中集内=${affected.length} 张：${affected.map((id) => `${pres.rows.find((r) => String(r.id) === id)?.nameZh}(${id}) T18保留观测 con${prevCards.get(id)?.current?.console} pc${prevCards.get(id)?.current?.pc} → 复算时判为不适用`).join(' | ')}`);

// 真实进/出（以 rows 自洽复算的 T18 命中集为基线）
...
P(`[真实变动] 建议集 T18 ${hitPrevRows.length} → T19 ${hitNowRows.length}（进 ${inNow.length} 出 ${outNow.length}）`);
