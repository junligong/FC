// 一次性脚本（临时）：T10 轮「前、本轮命中集」按分析器真实输入（research rows）自洽复算，
// 作为「判定链无回归」的独立证据（不依赖 hourly 快照口径，避免部分失败轮造成的假差异）。
// 口径：命中集 = 从 research rows 的 fc27Current.representative / fc27Max / fc26Launch 复算。
// 输入：research（T10）+ research-prev-t09.json（2026-09-20T09）+ hourly 2026-09-20T09（仅用于核查该轮是否含未采集卡）。
...
const pres = j('automation/runs/2026-09-20/icons-heroes/work/research-prev-t09.json');
const hPrev = j(`apps/market/engine/icons/data/prices/fc27/pricerange/hourly/${D}T09.json`);
...
P(`[自洽复算] T09(前轮) 文件自判=${advPrevFile.length} 与 rows 复算=${hitPrevRows.length} 差异=${d(hitPrevRows, advPrevFile)}`);
P(`[自洽复算] T10(本轮) 文件自判=${advNowFile.length} 与 rows 复算=${hitNowRows.length} 差异=${d(hitNowRows, advNowFile)}`);
...
P(`[基线核查] T09 未采集卡=${unfetched.length} 张${unfetched.length ? '：' + unfetched.map((id) => pres.rows.find((r) => String(r.id) === id)?.nameZh || id).join(' / ') : '（T09 为全量成功轮 ⇒ hourly 基线本轮同样可信）'}`);
const affected = unfetched.filter((id) => advPrevFile.includes(id));
P(`[基线核查] 未采集但在 T09 命中集内=${affected.length} 张${affected.length ? '：' + affected.map((id) => `${pres.rows.find((r) => String(r.id) === id)?.nameZh}(${id}) T09保留观测 con${prevCards.get(id)?.current?.console} pc${prevCards.get(id)?.current?.pc}`).join(' | ') : ''}`);

// 真实进/出（以 rows 自洽复算的 T09 命中集为基线）
const inNow = hitNowRows.filter((x) => !hitPrevRows.includes(x));
const outNow = hitPrevRows.filter((x) => !hitNowRows.includes(x));
P(`[真实变动] 建议集 T09 ${hitPrevRows.length} → T10 ${hitNowRows.length}（进 ${inNow.length} 出 ${outNow.length}）`);
