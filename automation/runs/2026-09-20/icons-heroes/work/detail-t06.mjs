// 一次性明细脚本（临时）：icons-pricerange-hourly 2026-09-20 T06 轮建议集变动与临界余量明细。
// 全部按 cardId 关联（FUTBIN slug 不唯一，禁用 slug/name 做键）。
// T06 本轮为全量成功轮（131/131，无 ok=false）：本轮取值口径 = current.json（分析器实际读取的源），
// 前轮取值 = 2026-09-20T05 逐小时快照（同为全量成功轮，同日前一轮）。
// 双映射：复算前轮(2026-09-20T05)命中集用生成该轮产物时的同一版 FC26 映射（research-prev-t05.json）。
