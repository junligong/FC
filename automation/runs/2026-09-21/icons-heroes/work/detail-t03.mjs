// 一次性脚本（临时）：icons-pricerange-hourly 2026-09-21 T03 轮明细（进/出、临界、极值集、改动面、红线）。
// 只读。全部按 cardId。
...
P('\n===== 改动面（绝对时刻 2026-09-21 03:41:00 之后，排除 work/ .git .workbuddy） =====');
const exec = (cmd) => {
  const r = require('node:child_process').execSync(cmd, { cwd: ROOT, encoding: 'utf8' });
  return r.trim().split('\n').filter(Boolean);
};
const changed = exec(`find . -type f -newermt '2026-09-21 03:41:00' -not -path './.git/*' -not -path './.workbuddy/*' -not -path '*/work/*' -not -path './node_modules/*'`);
changed.forEach((f) => {
...
for (const f of [
  ...
  `automation/runs/${D}/icons-heroes/hourly-03-failed.json`,
]) {
